...
Position yellow_;
  ...
  p_and_s()
  {
   Pub_point = nh.advertise<geometry_msgs::PoseArray>("/point",50);//队列的长度要一致，不然数据点对不上号
    ...
    if(c_flag == 0 && X != 0)
      {
          ...
          p.position.x = (yellow[0].x + yellow[1].x)/2;
          p.position.y = (yellow[0].y + yellow[1].y)/2;
          center.poses.push_back(p);
          for (uint8_t i = 0; i < blue.size(); i++)
          {
            p.position.x = (blue[i].x + red[i].x)/2;
            p.position.y = (blue[i].y + red[i].y)/2;
            center.poses.push_back(p);
          }
          count3 = 1;
...
{
  ros::init(argc,argv,"CNode");//  初始化节点
