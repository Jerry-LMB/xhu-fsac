...
// Generated 11-Aug-2021 23:59:09
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"Motion_Planning\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(Motion_Planning_M, (NULL));
        Motion_Planning_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(Motion_Planning_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(Motion_Planning_M)
                == (NULL)) || rtmGetStopRequested(Motion_Planning_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(Motion_Planning_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
