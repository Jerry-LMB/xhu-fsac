...
// Generated 13-Aug-2021 15:33:20
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"Controller_PP\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(Controller_PP_M, (NULL));
        Controller_PP_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(Controller_PP_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(Controller_PP_M)
                == (NULL)) || rtmGetStopRequested(Controller_PP_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(Controller_PP_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
