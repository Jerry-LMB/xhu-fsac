...
void CallBack(const darknet_ros_msgs:: BoundingBoxes::ConstPtr &msg)
{
  ...
  ///////////// 下面开始就是存储和偏移的中心点///////////////////////////////////////
    ...
    if(c_flag == 0 && X != 0)
      {
            p.position.x = X;
            p.position.y = Y;
            center.poses.push_back(p);//放的自身的坐标点
          
          p.position.x = (blue[0].x + red[0].x)/2;
          p.position.y = (blue[0].y + red[0].y)/2;
          center.poses.push_back(p);
          p.position.x = (blue[1].x + red[1].x)/2;
          p.position.y = (blue[1].y + red[1].y)/2;
          center.poses.push_back(p);//看到的前两个锥桶
          float aa = 3.80;
          float th = 0.0;
          float bb = 0.4186;
          float cc = 5.4;
          th = atan((center.poses[2].position.y - center.poses[1].position.y)/(center.poses[2].position.x - center.poses[1].position.x));
          p.position.x = center.poses[2].position.x + cc*cos(th);
          p.position.y = center.poses[2].position.y + cc*sin(th);
          center.poses.push_back(p);//八字的中心锥桶
          for (uint8_t i = 3; i < 18; i++)
          {
            /* code */
            p.position.x = center.poses[i].position.x + aa*cos(th - (i-2)*bb);
            p.position.y = center.poses[i].position.y + aa*sin(th - (i-2)*bb);
            center.poses.push_back(p);
          }
          for (uint8_t i = 18; i < 34; i++)
          {
            /* code */
            p.position.x = center.poses[i].position.x + aa*cos(th + (i-17)*bb);
            p.position.y = center.poses[i].position.y + aa*sin(th + (i-17)*bb);
            center.poses.push_back(p);
          }
          count3 = 1;
    ...
    { 
      ...
      //     float th = 0.0;
      //     float bb = 0.4186;
      //     th = atan((center.poses[1].position.y - center.poses[0].position.y)/(center.poses[1].position.x - center.poses[0].position.x));

      //     for (uint8_t i = 1; i < 16; i++)
      //     {
      //       /* code */
      //       p.position.x = center.poses[i].position.x + aa*cos(th - i*bb);
      //       p.position.y = center.poses[i].position.y + aa*sin(th - i*bb);
      //       center.poses.push_back(p);
      //     }
      //     count3 = 1;
      //   }
...
{
  ros::init(argc,argv,"ENode");//  初始化节点
